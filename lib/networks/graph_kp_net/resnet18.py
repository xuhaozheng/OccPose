...
'''
kp net added with graph
'''

class Resnet18(nn.Module):
    def __init__(self, ver_dim, seg_dim, graph_dim, fcdim=256, s8dim=128, s4dim=64, s2dim=32, raw_dim=32):
        ...
        self.convraw = nn.Sequential(
            ...
            nn.LeakyReLU(0.1,True),
            nn.Conv2d(raw_dim, seg_dim+ver_dim+graph_dim, 1, 1)
        )
    ...
    def forward(self, x, feature_alignment=False):
        ...
        ver_pred=x[:,self.seg_dim:(self.ver_dim+self.seg_dim),:,:]
        graph_pred = x[:,(self.ver_dim+self.seg_dim):,:,:]

        ret = {'seg': seg_pred, 'vertex': ver_pred, 'graph':graph_pred}
