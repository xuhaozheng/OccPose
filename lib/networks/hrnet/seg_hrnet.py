...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.last_layer = nn.Sequential(
            nn.Conv2d(last_inp_channels, self.raw_dim, 3, 1, 1, bias=False),
            nn.BatchNorm2d(self.raw_dim),
            nn.LeakyReLU(0.1,True),
            nn.Conv2d(self.raw_dim, self.seg_dim+self.ver_dim, 1, 1)
        )
    ...
    def forward(self, x):
        ...
        # print("input size before last layer",x.shape)

        x = self.last_layer(x)

        # print("input size after last layer",x.shape)

        seg_pred=x[:,:self.seg_dim,:,:]
        ver_pred=x[:,self.seg_dim:,:,:]
        # print("ver pred",ver_pred.shape)
    ...
    def init_weights(self, pretrained='',):
        ...
        if os.path.isfile(pretrained):
            ...
            print('=> pretrained model not exist!!! {}'.format(pretrained))
