...
def train(cfg, network):
    ...
    # print('num of train images', len(train_loader))
    val_loader = make_data_loader(cfg, is_train=False)
    # print('num of val images', len(val_loader))
    # train_loader = make_data_loader(cfg, is_train=True, max_iter=100)
    # cfg.save_ep = 1
    print("every epoch for save:",cfg.save_ep)
    ...
    for epoch in range(begin_epoch, cfg.train.epoch):
        ...
        if (epoch + 1) % (cfg.save_ep) == 0:
            save_model(network, optimizer, scheduler,
                       recorder, epoch, cfg.model_dir)
